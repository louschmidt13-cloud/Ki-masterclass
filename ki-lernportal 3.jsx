...
export default function App() {
    ...
    (async () => {
      try {
        const [d, c, n] = await Promise.all([
          window.storage.get("ki-done"),
          window.storage.get("ki-checks"),
          window.storage.get("ki-notes"),
        ]);
  ...
  const saveData = useCallback(async (d, c, n) => {
    try {
      await Promise.all([
        window.storage.set("ki-done", JSON.stringify(d)),
        window.storage.set("ki-checks", JSON.stringify(c)),
        window.storage.set("ki-notes", JSON.stringify(n)),
      ]);
    } catch (e) { /* ignore */ }
  }, []);
